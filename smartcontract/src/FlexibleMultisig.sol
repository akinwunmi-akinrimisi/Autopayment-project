...
contract FlexibleMultisig {
    uint256 public quorum;
    address[] public signers;
    IERC20 public feeToken; 

    enum ProposalType {  AddSigner, RemoveSigner, UpdateQuorum, WithdrawFees }
    struct Proposal {
    ...
    function createProposal(
        ...
        // proposal.milestoneId = milestoneId;
        // proposal.refundAmount = refundAmount;
        // proposal.releaseAmount = releaseAmount;
    ...
    function vote(uint256 proposalId, bool support) external onlySigner {
    ...
    function executeProposal(uint256 proposalId) external onlySigner {
        ...
        if (proposal.votesFor <= proposal.votesAgainst) revert HigherVotesAgainst();

        // if (proposal.proposalType == ProposalType.ExecuteTransaction) {
        //     IEscrow(proposal.target).settleMilestone(
        //         proposal.milestoneId,
        //         proposal.refundAmount,
        //         proposal.releaseAmount
        //     );
        // } else
         if (proposal.proposalType == ProposalType.AddSigner) {
            ...
            if (proposal.newQuorum == 0 || proposal.newQuorum > signers.length) revert InvalidQuorum();
            quorum = proposal.newQuorum;
        } else if (proposal.proposalType == ProposalType.WithdrawFees) {
            if (proposal.withdrawalAmount > feeToken.balanceOf(address(this))) revert InsufficientTokenBalance();
            feeToken.transfer(proposal.target, proposal.withdrawalAmount);
        }
    ...
    // New function to settle milestones without quorum (only signer required)
    function settleMilestoneWithoutQuorum(
        ...
        IEscrow(escrowContract).settleMilestone(milestoneId, refundAmount, releaseAmount);
